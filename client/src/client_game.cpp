...
#define MAP_RECEIVED_NAME "/tmp/worms/map.tar.gz"
#define MAP_YML_PATH "/tmp/worms/map.yml"
...
team_id(tid) {
	...
	file_map.close();
	std::string cmd_unzip_tar_gz = "tar -xf " + map_received_name + " -C /tmp/worms/";
...
team_id(tid) {
	...
	std::string cmd_unzip_tar_gz = "tar -xf " + mp + " -C /tmp/worms/";
	std::system(cmd_unzip_tar_gz.c_str());
	this->mapNode = YAML::LoadFile(MAP_YML_PATH);
	this->creator = true;
}
...
void ClientGame::removeTempFiles(void) {
	if (!this->creator) {
		std::string map_received_name(MAP_RECEIVED_NAME);
		std::string cmd_rm_map_yml = "rm /tmp/worms/map.yml /tmp/worms/background.png " + map_received_name;
		std::system(cmd_rm_map_yml.c_str());
	} else {
		std::string cmd_rm_map_yml = "rm /tmp/worms/map.yml /tmp/worms/background.png";
		std::system(cmd_rm_map_yml.c_str());		
	}
}
